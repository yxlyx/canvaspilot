...
from app.schemas.source_imports import SourceImportRun, SourceParseRun
...
from app.services.source_imports import (
    import_ingestion_job_sources,
    parse_and_import_ingestion_job_sources,
)
...
@router.post("/{job_id}/run", response_model=IngestionJobResponse)
async def run_ingestion_job(
    job_id: uuid.UUID,
    payload: SourceImportRun,
    user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    return await import_ingestion_job_sources(user, job_id, payload.sources, db)


@router.post("/{job_id}/parse-run", response_model=IngestionJobResponse)
async def parse_and_run_ingestion_job(
    job_id: uuid.UUID,
    payload: SourceParseRun,
    user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    return await parse_and_import_ingestion_job_sources(user, job_id, payload.sources, db)
