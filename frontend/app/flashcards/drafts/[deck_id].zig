...
fn renderReview(req: mer.Request, deck: lib.types.FlashcardDeckResponse, history: []const lib.types.FlashcardRevisionResponse) mer.Response {
    ...
    for (deck.cards) |card| if (!std.mem.eql(u8, card.state, "discarded")) {
        ...
        const low_signal_card = isLowSignalCard(card.question, card.answer);
        if (card.approved) approved += 1;
        if (unsupported_card) unsupported += 1;
        if (low_signal_card) low_signal += 1;
        if (!card.approved or unsupported_card or low_signal_card) attention += 1;
    };
    ...
    for (deck.cards) |card| {
        ...
        const unsupported_card = !card.manual_note and (card.source_id == null or (card.source_chunk_id == null and card.wiki_page_id == null));
        const needs_attention = !discarded and (!card.approved or unsupported_card or isLowSignalCard(card.question, card.answer));
...
fn renderCard(req: mer.Request, w: *std.Io.Writer, card: lib.types.FlashcardResponse, scope: std.json.Value, editable: bool, open: bool) !void {
    ...
    const approvable = editable and !discarded and !low_signal and !unsupported;
    const disabled = if (editable and !discarded) "" else " disabled";
    const state_class: []const u8 = if (discarded) "neutral" else if (low_signal or unsupported) "warn" else if (card.approved) "ok" else "info";
    const state_label: []const u8 = if (discarded) "Discarded" else if (low_signal) "Needs rewrite" else if (unsupported) "Needs evidence" else if (card.approved) "Approved" else "Needs approval";
    ...
    if (low_signal and !discarded) try w.writeAll("<div class=\"notice notice-warn\" role=\"alert\"><strong>This card tests document metadata.</strong> Rewrite it around a useful course concept or discard it.</div>");
