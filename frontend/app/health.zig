...
pub fn render(req: mer.Request) mer.Response {
    ...
    const w = &buf.writer;
    w.writeAll("<div class=\"wb-m3-page wb-m3-health page-grid\"><header class=\"cp-page-header wb-m3-header\"><div><h1 class=\"cp-page-title\">Workspace health</h1><p class=\"cp-page-sub\">Structured checks for citation coverage, stale pages, and failed references.</p></div><div class=\"cp-disabled-action wb-m3-header-action\"><button class=\"cp-btn cp-btn-primary\" type=\"button\" aria-disabled=\"true\" aria-describedby=\"health-run-note\">Run checks</button><small id=\"health-run-note\">Unavailable: live checks require a backend computation endpoint.</small></div></header>") catch return mer.internalError("health render failed");
    ...
    for (lib.mock.health_findings) |finding| {
        ...
        w.print("<article class=\"cp-card cp-finding surface wb-m3-finding\"><div><span class=\"cp-state cp-state-{s} status-pill wb-m3-state\">{s}</span> <span>{s}</span></div><h2>{s}</h2><p>{s}</p><p><strong>Recommendation:</strong> {s}</p><a class=\"cp-btn cp-btn-ghost\" href=\"{s}\">{s}</a></article>", .{ @tagName(finding.state), @tagName(finding.state), subject, title, detail, recommendation, action_href, action_label }) catch return mer.internalError("health render failed");
    }
