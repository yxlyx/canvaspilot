...
pub fn render(req: mer.Request) mer.Response {
    ...
    const w = &buf.writer;
    w.writeAll("<header class=\"cp-page-header wb-m3-history-header\"><div><h1 class=\"cp-page-title\">History</h1><p class=\"cp-page-sub\">Version timeline with citation changes and a readable unified diff.</p></div></header>") catch return mer.internalError("history render failed");
    ...
    for (lib.mock.history_changes) |change| {
        ...
        w.print("</pre><button class=\"cp-btn cp-btn-ghost button button-secondary\" type=\"button\" aria-disabled=\"true\" aria-describedby=\"history-note-{s}\">Restore version</button><small id=\"history-note-{s}\">Restore is unavailable until a backend mutation contract exists.</small></article></li>", .{ change_id, change_id }) catch return mer.internalError("history render failed");
    }
