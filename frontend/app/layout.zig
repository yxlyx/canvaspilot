...
pub fn wrap(allocator: std.mem.Allocator, path: []const u8, body: []const u8, meta: mer.Meta) []const u8 {
    ...
    w.writeAll(
        ...
        \\<script>try{var p=localStorage.getItem('wikibase-theme-preference'),t=localStorage.getItem('wikibase-theme');document.documentElement.dataset.theme=p==='system'?(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light'):(p||t||(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light'))}catch(e){}</script>
        \\<style>
    ) catch return body;
