...
pub fn render(req: mer.Request) mer.Response {
    ...
    const w = &buf.writer;
    w.writeAll("<header class=\"cp-page-header\"><div><h1 class=\"cp-page-title\">Marked papers</h1><p class=\"cp-page-sub\">Privacy-sensitive evidence review. Extraction creates proposals, never confirmed knowledge.</p></div><button class=\"cp-btn cp-btn-primary\" type=\"button\" disabled>Upload paper</button></header>") catch return mer.internalError("marked papers render failed");
    lib.m3.demoBanner(w) catch return mer.internalError("marked papers render failed");
    ...
    for (lib.mock.marked_papers) |paper| {
        ...
        const confidence = lib.ui.escape(req.allocator, paper.extraction_confidence) catch paper.extraction_confidence;
