...
fn renderLive(req: mer.Request) mer.Response {
    ...
    w.print("</select></label><label class=\"cp-field\" data-scope-field=\"topic\"{s}><span>Topic</span><input name=\"topic\" maxlength=\"100\" autocomplete=\"off\" required{s}></label><label class=\"cp-field\"><span>Optional title</span><input name=\"title\" maxlength=\"300\"></label><button class=\"cp-btn cp-btn-primary\" type=\"submit\">Generate cited output</button><p class=\"cp-form-status\" role=\"status\" aria-live=\"polite\" tabindex=\"-1\"></p></form>", .{ if (std.mem.eql(u8, initial_scope, "topic")) "" else " hidden", if (std.mem.eql(u8, initial_scope, "topic")) "" else " disabled" }) catch return mer.internalError("outputs render failed");
