...
fn renderLive(req: mer.Request) mer.Response {
    ...
    var buf = lib.ui.buildHtml(req.allocator);
    const w = &buf.writer;
    w.writeAll("<header class=\"cp-page-header\"><div><h1 class=\"cp-page-title\">Cited study outputs</h1><p class=\"cp-page-sub\">Generate grounded summaries, outlines, or study guides from exactly one workspace scope.</p></div></header>" ++
        "<section class=\"cp-card\" aria-labelledby=\"generate-title\"><h2 id=\"generate-title\">Create output</h2><form id=\"cp-output-form\" class=\"cp-selector-grid\" method=\"post\" action=\"/api/m3\" data-m3-form data-success=\"/outputs\">" ++
    ...
    w.print("<option value=\"source_ids\"{s}>Sources</option><option value=\"wiki_page_id\"{s}>Wiki page</option><option value=\"topic\">Topic</option></select></label><label class=\"cp-field\"><span>Source</span><select name=\"source_ids\"{s}><option value=\"\">Choose a source</option>", .{ if (source_available) "" else " disabled", if (page_available) "" else " disabled", if (source_available) "" else " disabled" }) catch return mer.internalError("outputs render failed");
    ...
    w.print("</select></label><label class=\"cp-field\"><span>Wiki page</span><select name=\"wiki_page_id\"{s}><option value=\"\">Choose a page</option>", .{if (page_available) "" else " disabled"}) catch return mer.internalError("outputs render failed");
    ...
    w.writeAll("</select></label><label class=\"cp-field\"><span>Topic</span><input name=\"topic\" maxlength=\"100\" autocomplete=\"off\"></label><label class=\"cp-field\"><span>Optional title</span><input name=\"title\" maxlength=\"300\"></label><button class=\"cp-btn cp-btn-primary\" type=\"submit\">Generate cited output</button><p class=\"cp-form-status\" role=\"status\" aria-live=\"polite\" tabindex=\"-1\"></p></form>") catch return mer.internalError("outputs render failed");
    if (sources.value == null) w.writeAll("<p class=\"cp-unavailable\" role=\"alert\">Sources are unavailable. Source scope is disabled; topic and available wiki scopes still work.</p>") catch return mer.internalError("outputs render failed") else if (!source_available) w.writeAll("<p class=\"cp-muted-copy\">No sources are available, so source scope is disabled.</p>") catch return mer.internalError("outputs render failed");
    if (pages.value == null) w.writeAll("<p class=\"cp-unavailable\" role=\"alert\">Wiki pages are unavailable. Wiki scope is disabled; topic and available source scopes still work.</p>") catch return mer.internalError("outputs render failed") else if (!page_available) w.writeAll("<p class=\"cp-muted-copy\">No wiki pages are available, so wiki scope is disabled.</p>") catch return mer.internalError("outputs render failed");
    w.writeAll("</section><section aria-labelledby=\"saved-title\"><h2 id=\"saved-title\">Saved outputs</h2><div class=\"cp-list-grid\">") catch return mer.internalError("outputs render failed");
    if (outputs.value.?.value.items.len == 0) w.writeAll("<div class=\"cp-empty\"><h3>No outputs yet</h3><p>Choose one scope above to create the first output.</p></div>") catch return mer.internalError("outputs render failed");
    for (outputs.value.?.value.items[0..@min(outputs.value.?.value.items.len, 20)]) |output| {
        ...
        w.print("<article class=\"cp-card\"><div class=\"cp-card-title\"><h3><a href=\"/outputs/{s}\">{s}</a></h3><span class=\"cp-state\">{s}</span></div><p>{s}</p><p class=\"cp-muted-copy\">{s} · {d} grounded citation(s)</p></article>", .{ lib.ui.escapeSafe(req.allocator, output.id), lib.ui.escapeSafe(req.allocator, output.title), lib.ui.escapeSafe(req.allocator, boundary), lib.ui.escapeSafe(req.allocator, output.message), lib.ui.escapeSafe(req.allocator, output.output_type), output.citations.len }) catch return mer.internalError("outputs render failed");
    }
    ...
    w.writeAll("</nav><script src=\"/m3.js\" defer></script>") catch return mer.internalError("outputs render failed");
    return lib.ui.htmlResponse(&buf);
}
