...
fn renderLive(req: mer.Request) mer.Response {
    ...
    var buf = lib.ui.buildHtml(req.allocator);
    const w = &buf.writer;
    w.writeAll("<div class=\"wb-m3-page wb-m3-progress page-grid\"><header class=\"cp-page-header wb-m3-header\"><div><h1 class=\"cp-page-title\">Knowledge meter</h1><p class=\"cp-page-sub\">Legacy factual signals. Completion and confidence remain unscored; use an enrollment dashboard for separated metrics.</p></div></header><section class=\"cp-meter-grid wb-m3-meter-grid\">") catch return mer.internalError("meter render failed");
    for (meters) |meter| {
        ...
        w.writeAll("<p class=\"cp-unknown-meter\"><strong>No legacy completion score.</strong> Use an enrollment dashboard for separate factual metrics.</p>") catch return mer.internalError("meter render failed");
        w.print("<dl class=\"cp-facts wb-m3-facts\"><div><dt>Evidence</dt><dd>{d} factual signal(s)</dd></div><div><dt>Score status</dt><dd>Not authoritative</dd></div><div><dt>Freshness</dt><dd>{s}</dd></div></dl><h3>Contributing signals</h3><ul class=\"cp-plain-list wb-m3-signals\">", .{ meter.evidence_count, if (meter.stale) "Stale" else "Current" }) catch return mer.internalError("meter render failed");
        ...
        w.print("</ul><aside class=\"cp-recommendation wb-m3-recommendation\"><h3>Compatibility notice</h3><p>{s}</p><div class=\"cp-action-row wb-m3-actions\"><a class=\"cp-btn cp-btn-ghost\" href=\"/settings/learning\">Open enrollment dashboards</a></div></aside></article>", .{lib.ui.escapeSafe(req.allocator, meter.recommendation)}) catch return mer.internalError("meter render failed");
    }
