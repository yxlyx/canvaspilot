...
pub fn render(req: mer.Request) mer.Response {
    ...
    w.writeAll("<section class=\"cp-card cp-progress-overview\" aria-labelledby=\"progress-overview-title\"><div class=\"cp-card-title\"><h2 id=\"progress-overview-title\">Workspace evidence overview</h2><span>aggregate estimate</span></div>") catch return mer.internalError("progress render failed");
    ...
    for (lib.mock.recommendations) |recommendation| {
        ...
        w.print("<article class=\"cp-recommendation\"><h3>{s}</h3><p><strong>Why:</strong> {s}</p><p><strong>Evidence:</strong> {s}</p><p><strong>Next action:</strong> {s}</p><div class=\"cp-action-row\">", .{ title, why, evidence, action }) catch return mer.internalError("progress render failed");
...
fn renderLive(req: mer.Request) mer.Response {
    ...
    var buf = lib.ui.buildHtml(req.allocator);
    const w = &buf.writer;
    w.writeAll("<header class=\"cp-page-header\"><div><h1 class=\"cp-page-title\">Knowledge meter</h1><p class=\"cp-page-sub\">Backend-measured topic evidence. Unknown values remain unknown—no live percentage is invented.</p></div></header><section class=\"cp-meter-grid\">") catch return mer.internalError("meter render failed");
    for (meters) |meter| {
        w.print("<article class=\"cp-card cp-meter-card\"><div class=\"cp-card-title\"><h2>{s}</h2><span class=\"cp-state cp-state-{s}\">{s}</span></div>", .{ lib.ui.escapeSafe(req.allocator, meter.topic), lib.ui.escapeSafe(req.allocator, meter.state), lib.ui.escapeSafe(req.allocator, meter.state) }) catch return mer.internalError("meter render failed");
        ...
        w.print("<dl class=\"cp-facts\"><div><dt>Evidence</dt><dd>{d} observations</dd></div><div><dt>Confidence</dt><dd>{d:.0}%</dd></div><div><dt>Freshness</dt><dd>{s}</dd></div></dl><h3>Contributing signals</h3><ul class=\"cp-plain-list\">", .{ meter.evidence_count, meter.evidence_confidence * 100, if (meter.stale) "Stale" else "Current" }) catch return mer.internalError("meter render failed");
        ...
        w.print("</ul><aside class=\"cp-recommendation\"><h3>Recommended next step</h3><p>{s}</p><div class=\"cp-action-row\"><a href=\"/wiki\">Review notes</a><a href=\"/flashcards\">Practise cards</a><a href=\"/marked-papers\">Review evidence</a></div></aside></article>", .{lib.ui.escapeSafe(req.allocator, meter.recommendation)}) catch return mer.internalError("meter render failed");
    }
