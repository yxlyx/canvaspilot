...
fn renderLive(req: mer.Request) mer.Response {
    ...
    w.writeAll("<p class=\"cp-provider-intro\">Choose one connection for cited answers. Source search and reading remain available when no answer provider is connected.</p><div class=\"cp-provider-workspace\"><nav class=\"cp-provider-list\" aria-label=\"AI providers\">") catch return mer.internalError("providers render failed");
    ...
    if (descriptors.len == 0) w.writeAll("</nav><section class=\"cp-provider-active\"><div class=\"cp-empty\"><h2>No answer providers are available</h2><p>Your sources and Wiki still work. Reload after the local backend configuration is restored.</p><a class=\"cp-btn cp-btn-primary\" href=\"/settings/providers\">Retry provider settings</a></div></section></div>") catch return mer.internalError("providers render failed") else {
        ...
        w.print("</nav><section class=\"cp-provider-active\"><article class=\"cp-card cp-provider-card surface wb-m3-provider-card\"><div class=\"cp-card-title wb-m3-card-head\"><div><p class=\"eyebrow\">Generation provider</p><h2>{s}</h2></div><strong class=\"cp-state status-pill cp-state-{s} wb-m3-status\">{s}</strong></div><p class=\"cp-provider-description\">{s}</p><details class=\"cp-provider-details\"><summary>Capabilities, billing, and data use</summary><ul class=\"cp-provider-capabilities\">", .{ lib.ui.escapeSafe(req.allocator, provider.name), lib.ui.escapeSafe(req.allocator, status_class), lib.ui.escapeSafe(req.allocator, status), lib.ui.escapeSafe(req.allocator, provider.description) }) catch return mer.internalError("providers render failed");
        ...
        w.writeAll("</aside><p>When active, the provider receives your question, recent chat turns, and selected source excerpts. During connection, workspace questions and sources are not sent. Credentials remain server-side or inside the local CLI.</p></details>") catch return mer.internalError("providers render failed");
        ...
        if (oauthMethod(provider)) |auth_method| {
            const local_auth_method = authMethod(provider, "local_cli");
            const prefer_local = local_auth_method != null and local_auth_method.?.enabled and !auth_method.enabled;
            if (current) |setting| if (setting.provider_account_label) |account| w.print("<dl class=\"cp-provider-account\"><div><dt>Connected account</dt><dd>{s}</dd></div><div><dt>Authentication</dt><dd>{s}</dd></div><div><dt>Model</dt><dd>{s}</dd></div></dl>", .{ lib.ui.escapeSafe(req.allocator, account), if (std.mem.eql(u8, setting.auth_method, "local_cli")) "Local Codex CLI" else "Browser sign-in", lib.ui.escapeSafe(req.allocator, setting.model) }) catch return mer.internalError("providers render failed");
            w.print("<div class=\"cp-provider-methods{s}\">", .{if (prefer_local) " cp-provider-prefer-local" else ""}) catch return mer.internalError("providers render failed");
            ...
            if (local_auth_method) |local_method| {
                w.print("<form class=\"wb-m3-form cp-provider-setup cp-provider-local\" method=\"post\" action=\"/api/m3\" data-m3-form data-success=\"/settings/providers?provider=chatgpt\"><input type=\"hidden\" name=\"action\" value=\"provider.local.connect\"><input type=\"hidden\" name=\"id\" value=\"chatgpt\"><div class=\"cp-provider-auth-choice\"><span class=\"cp-provider-auth-icon\" aria-hidden=\"true\">⌘</span><div><strong>{s}</strong><small>Validates the Codex CLI login on this computer. CanvasPilot does not read or store its tokens.</small></div></div><button class=\"cp-btn cp-btn-ghost\" type=\"submit\"{s}>{s}</button><p class=\"cp-form-status wb-m3-form-status\" role=\"status\"></p></form>", .{ lib.ui.escapeSafe(req.allocator, local_method.label), if (local_method.enabled) "" else " disabled aria-disabled=\"true\"", if (current != null and std.mem.eql(u8, current.?.auth_method, "local_cli")) "Revalidate local Codex CLI" else "Use local Codex CLI" }) catch return mer.internalError("providers render failed");
                if (!local_method.enabled) if (local_method.unavailable_reason) |reason| w.print("<p class=\"cp-muted-copy cp-provider-unavailable cp-provider-local-unavailable\">{s}</p>", .{lib.ui.escapeSafe(req.allocator, reason)}) catch return mer.internalError("providers render failed");
            }
...
fn authMethod(provider: lib.types.ProviderDescriptor, kind: []const u8) ?lib.types.ProviderAuthMethodDescriptor {
    for (provider.auth_methods) |method| if (std.mem.eql(u8, method.kind, kind)) return method;
    return null;
}

fn oauthMethod(provider: lib.types.ProviderDescriptor) ?lib.types.ProviderAuthMethodDescriptor {
    return authMethod(provider, "oauth_code");
}
