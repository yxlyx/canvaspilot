...
fn renderQuestion(req: mer.Request, w: *std.Io.Writer, paper_id: []const u8, question: lib.types.MarkedPaperQuestionResponse) !void {
    const question_id = lib.ui.escapeSafe(req.allocator, question.id);
    try w.print("<article><form method=\"post\" action=\"/api/m3\" data-m3-form data-success=\"\"><input type=\"hidden\" name=\"action\" value=\"paper.updateQuestion\"><input type=\"hidden\" name=\"id\" value=\"{s}\"><input type=\"hidden\" name=\"child_id\" value=\"{s}\"><div><span class=\"cp-state status-pill\">{s}</span><h3>Question {d}</h3></div><label class=\"cp-field\"><span>Question text</span><textarea name=\"question_text\" required>{s}</textarea></label><div class=\"cp-settings-inline\"><label class=\"cp-field\"><span>Awarded marks</span><input type=\"number\" min=\"0\" step=\"0.5\" name=\"awarded_marks\" value=\"{s}\"></label><label class=\"cp-field\"><span>Available marks</span><input type=\"number\" min=\"0.5\" step=\"0.5\" name=\"available_marks\" value=\"{s}\"></label><label class=\"cp-field\"><span>Topic</span><input name=\"topic_tag\" value=\"{s}\" required></label><label class=\"cp-field\"><span>Confidence</span><input type=\"number\" min=\"0\" max=\"1\" step=\"0.01\" name=\"confidence\" value=\"{d:.2}\"></label></div><label class=\"cp-field\"><span>Feedback</span><textarea name=\"feedback\">{s}</textarea></label><label class=\"cp-check-row\"><input type=\"checkbox\" name=\"reviewed\"{s}><span>Reviewed and confirmed</span></label><button class=\"cp-btn cp-btn-primary\" type=\"submit\">Save review</button><p class=\"cp-form-status\" role=\"status\"></p></form><form method=\"post\" action=\"/api/m3\" data-m3-form data-confirm=\"Delete this extracted question?\" data-success=\"\"><input type=\"hidden\" name=\"action\" value=\"paper.deleteQuestion\"><input type=\"hidden\" name=\"id\" value=\"{s}\"><input type=\"hidden\" name=\"child_id\" value=\"{s}\"><button class=\"cp-btn cp-btn-danger\" type=\"submit\">Delete question</button><p class=\"cp-form-status\" role=\"status\"></p></form></article>", .{ paper_id, question_id, if (question.reviewed) "reviewed" else "proposal", question.question_number, lib.ui.escapeSafe(req.allocator, question.question_text), number(req, question.awarded_marks), number(req, question.available_marks), lib.ui.escapeSafe(req.allocator, question.topic_tag), question.confidence, lib.ui.escapeSafe(req.allocator, question.feedback), if (question.reviewed) " checked" else "", paper_id, question_id });
}
