...
pub fn render(req: mer.Request) mer.Response {
    ...
    if (use_mock) {
        ...
        if (std.mem.eql(u8, selected, "all") or std.mem.eql(u8, selected, "evidence")) {
            shown += 1;
            const timestamp = "2026-05-17T18:25:00Z";
            const relative = lib.time.formatRelative(req.allocator, timestamp, now_secs) catch "—";
            const absolute = lib.time.formatAbsolute(req.allocator, timestamp) catch "—";
            w.print("<li><article class=\"wb-m3-history-card\"><time datetime=\"{s}\" title=\"{s}\">{s}</time><div><p class=\"eyebrow\">Flashcard review</p><h2>Balanced trees evidence recorded</h2><p>Reviewed 8 cards with 75% recall; the result now contributes to the topic estimate.</p></div></article></li>", .{ timestamp, absolute, relative }) catch return mer.internalError("activity render failed");
        }
        if (std.mem.eql(u8, selected, "all") or std.mem.eql(u8, selected, "study_guides")) {
            shown += 1;
            const timestamp = "2026-05-16T11:10:00Z";
            const relative = lib.time.formatRelative(req.allocator, timestamp, now_secs) catch "—";
            const absolute = lib.time.formatAbsolute(req.allocator, timestamp) catch "—";
            w.print("<li><article class=\"wb-m3-history-card\"><time datetime=\"{s}\" title=\"{s}\">{s}</time><div><p class=\"eyebrow\">Study guide</p><h2>AVL rotations revision guide</h2><p>Generated from three indexed sources with citations retained.</p></div></article></li>", .{ timestamp, absolute, relative }) catch return mer.internalError("activity render failed");
        }
    } else {
...
fn renderRevisions(req: mer.Request, w: *std.Io.Writer, page_id: []const u8) !void {
    ...
    for (revisions) |revision| {
        ...
        const absolute = lib.time.formatAbsolute(req.allocator, revision.created_at) catch "—";
        try w.print("<article><div><h3>Revision {d} · {s}</h3><p>{s}</p></div><span class=\"cp-state status-pill\">{d} citations</span><time datetime=\"{s}\" title=\"{s}\">{s}</time></article>", .{ revision.revision_number, lib.ui.escapeSafe(req.allocator, revision.title), lib.ui.escapeSafe(req.allocator, revision.change_summary), revision.citation_count, lib.ui.escapeSafe(req.allocator, revision.created_at), lib.ui.escapeSafe(req.allocator, absolute), lib.ui.escapeSafe(req.allocator, relative) });
    }
...
test "processing activity labels are learner readable" {
    try std.testing.expectEqualStrings("Processing completed", eventLabel("processing_completed"));
    try std.testing.expectEqualStrings("Processing needs attention", eventLabel("processing_failure"));
    try std.testing.expectEqualStrings("Processing recovered", eventLabel("processing_recovered"));
    try std.testing.expectEqualStrings("Workspace update", eventLabel("unknown_internal_event"));
}
