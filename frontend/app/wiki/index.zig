...
fn renderLiveArticle(req: mer.Request, w: *std.Io.Writer, page: lib.types.WikiPageResponse, enrollment_scope: []const u8) !void {
    ...
    const created = lib.time.formatRelative(req.allocator, page.created_at, lib.time.nowSecs()) catch "—";
    try w.print("<a class=\"article-card surface\" href=\"{s}\" data-module=\"Workspace\" data-search=\"{s} {s}\"><div class=\"article-card-top\"><span class=\"article-glyph\">{s}</span><span class=\"status-pill status-{s}\">{s}</span></div><div><small>{s} · {d} sources · {d} citations</small><h2>{s}</h2><p>{s}</p></div><div class=\"topic-row\"><span>{d} backlinks</span><span>Created {s}</span></div><footer><span>Updated {s}</span><b>Read article {s}</b></footer></a>", .{ href, safe_title, safe_summary, ICON_BOOK, if (page.citation_count > 0) "good" else "warn", coverage, safe_type, page.source_ids.len, page.citation_count, safe_title, safe_summary, page.backlinks.len, created, when, ICON_ARROW });
}
