...
pub fn render(req: mer.Request) mer.Response {
    ...
    if (use_mock) {
        ...
        w.writeAll("</section><section class=\"cp-settings-ledger cp-knowledge-ledger\" aria-labelledby=\"topic-ledger\"><header><div><p class=\"eyebrow\">Illustrative evidence</p><h2 id=\"topic-ledger\">How topic evidence will read</h2></div></header>") catch return mer.internalError("knowledge render failed");
        ...
        const enrollments = if (result.value) |parsed| parsed.value else return lib.m3.liveError(req, "Knowledge", result.status);
        w.writeAll("<section class=\"wb-m3-overview cp-knowledge-summary\"><div><p class=\"eyebrow\">Current evidence</p><h2>Coverage, recall, and activity stay separate.</h2><p>Choose a module to see what your sources cover, what you have reviewed, and where evidence is still uncertain.</p></div><div class=\"cp-knowledge-ring cp-knowledge-ring-unknown\" role=\"img\" aria-label=\"Knowledge completion is not yet measured\"><strong>—</strong><span>not a mastery score</span></div></section><section class=\"cp-settings-ledger cp-knowledge-ledger\" aria-labelledby=\"module-ledger\"><header><div><p class=\"eyebrow\">Module evidence</p><h2 id=\"module-ledger\">Open a knowledge dashboard</h2></div></header>") catch return mer.internalError("knowledge render failed");
        var active_count: usize = 0;
        for (enrollments) |enrollment| {
            if (enrollment.archived) continue;
            active_count += 1;
            w.print("<article class=\"wb-m3-meter-card\"><div><p class=\"eyebrow\">{s} · Semester {d}</p><h3>{s}</h3><p>Source coverage, recall evidence, uncertainty, and recent learning activity for this module.</p></div><div><p class=\"cp-unknown-meter\"><strong>Evidence, not a blended score.</strong> Unknown values remain — until supporting activity exists.</p><div class=\"cp-action-row\"><a class=\"cp-btn cp-btn-primary\" href=\"/learning/{s}\">Open knowledge dashboard</a><a class=\"cp-btn cp-btn-ghost\" href=\"/sources?enrollment_id={s}\">Review module sources</a></div></div></article>", .{ lib.ui.escapeSafe(req.allocator, enrollment.code), enrollment.semester, lib.ui.escapeSafe(req.allocator, enrollment.title), lib.ui.escapeSafe(req.allocator, enrollment.id), lib.ui.escapeSafe(req.allocator, enrollment.id) }) catch return mer.internalError("knowledge render failed");
        }
        if (active_count == 0) w.writeAll("<div class=\"cp-empty\"><div><h3>No module evidence yet</h3><p>Import a module first, then add sources and review cards to build honest evidence.</p><a class=\"cp-btn cp-btn-primary\" href=\"/settings/learning\">Import a module</a></div></div>") catch return mer.internalError("knowledge render failed");
        w.writeAll("</section>") catch return mer.internalError("knowledge render failed");
    }
...
fn renderLiveMeter(req: mer.Request, w: *std.Io.Writer, meter: lib.types.TopicMeterResponse) !void {
    try w.print("<article class=\"wb-m3-meter-card\"><div><p class=\"eyebrow\">{s}</p><h3>{s}</h3><p>{d} factual signal(s) · {s} · {s}</p></div><div>", .{ lib.ui.escapeSafe(req.allocator, meter.state), lib.ui.escapeSafe(req.allocator, meter.topic), meter.evidence_count, if (meter.stale) "stale" else "current", lib.ui.escapeSafe(req.allocator, meter.reason_code orelse "legacy meter is non-authoritative") });
    try w.writeAll("<p class=\"cp-unknown-meter\"><strong>No legacy completion score.</strong> Open an enrollment dashboard for separate source coverage, self-reported recall, and activity.</p>");
    try w.writeAll("<ul class=\"cp-plain-list wb-m3-signals\">");
    ...
    try w.print("</ul><aside class=\"wb-m3-recommendation\"><strong>Compatibility notice</strong><p>{s}</p><div class=\"cp-action-row\"><a class=\"cp-btn cp-btn-ghost\" href=\"/settings/learning\">Open enrollment dashboards</a></div></aside></div></article>", .{lib.ui.escapeSafe(req.allocator, meter.recommendation)});
}
