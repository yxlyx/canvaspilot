...
pub fn render(req: mer.Request) mer.Response {
    ...
    if (use_mock) {
        ...
        for (enrollments) |enrollment| {
            ...
            active_count += 1;
            w.print("<article class=\"wb-m3-meter-card\"><div><p class=\"eyebrow\">{s} · Semester {d}</p><h3>{s}</h3><p>Source coverage, recall evidence, uncertainty, and recent learning activity for this module.</p></div><div><p class=\"cp-unknown-meter\"><strong>Evidence, not a blended score.</strong> Unknown values remain — until supporting activity exists.</p><div class=\"cp-action-row\"><a class=\"cp-btn cp-btn-primary\" href=\"/learning/{s}\">Open knowledge dashboard</a><a class=\"cp-btn cp-btn-ghost\" href=\"/sources\">Open source library</a></div></div></article>", .{ lib.ui.escapeSafe(req.allocator, enrollment.code), enrollment.semester, lib.ui.escapeSafe(req.allocator, enrollment.title), lib.ui.escapeSafe(req.allocator, enrollment.id) }) catch return mer.internalError("knowledge render failed");
        }
        if (active_count == 0) w.writeAll("<div class=\"cp-empty\"><div><h3>No module evidence yet</h3><p>Import a module first, then add sources and learning activity to build honest evidence.</p><a class=\"cp-btn cp-btn-primary\" href=\"/settings/learning\">Import a module</a></div></div>") catch return mer.internalError("knowledge render failed");
        w.writeAll("</section>") catch return mer.internalError("knowledge render failed");
    }
