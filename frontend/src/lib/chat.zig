...
pub fn aggregateSse(allocator: std.mem.Allocator, sse: []const u8) !Reply {
    ...
    while (frame_it.next()) |frame| {
        ...
        if (std.mem.eql(u8, event, "token")) {
            const parsed = std.json.parseFromSliceLeaky(TokenEvent, allocator, data, .{
                .ignore_unknown_fields = true,
                .allocate = .alloc_always,
            }) catch return error.InvalidSse;
            try message_buf.appendSlice(allocator, parsed.text);
        } else if (std.mem.eql(u8, event, "citations")) {
            const parsed = std.json.parseFromSliceLeaky(CitationsEvent, allocator, data, .{
                .ignore_unknown_fields = true,
                .allocate = .alloc_always,
            }) catch return error.InvalidSse;
            citations = parsed.citations;
        } else if (std.mem.eql(u8, event, "done")) {
            const parsed = std.json.parseFromSliceLeaky(DoneEvent, allocator, data, .{
                .ignore_unknown_fields = true,
                .allocate = .alloc_always,
            }) catch return error.InvalidSse;
            ...
            outcome = if (std.mem.eql(u8, parsed.outcome, "no_evidence")) "no_evidence" else "answer";
            done_seen = true;
        }
    }
    if (!done_seen) return error.InvalidSse;
    ...
    if (final_message.len == 0) return error.InvalidSse;
    return .{
