...
test "queued and blocked stages explain automatic processing and stale worker recovery" {
    const allocator = std.testing.allocator;
    const stages = [_]types.ProcessingStageResponse{
        .{ .id = "1", .name = "parse_index", .position = 0, .status = "queued", .attempt_count = 0, .max_attempts = 3, .available_at = "2000-01-01T00:00:00Z" },
        .{ .id = "2", .name = "wiki", .position = 3, .status = "blocked", .attempt_count = 0, .max_attempts = 3, .available_at = "2000-01-01T00:00:00Z" },
    };
    const run: types.ProcessingRunResponse = .{ .id = "323e4567-e89b-12d3-a456-426614174000", .source_id = "423e4567-e89b-12d3-a456-426614174000", .source_version_id = "523e4567-e89b-12d3-a456-426614174000", .status = "queued", .current_stage = "parse_index", .attempt_count = 0, .created_at = "2000-01-01T00:00:00Z", .updated_at = "2000-01-01T00:00:00Z", .stages = &stages };
    var out: std.Io.Writer.Allocating = .init(allocator);
    defer out.deinit();
    try render(allocator, &out.writer, &.{run}, "", false);
    const html = out.written();
    try std.testing.expect(std.mem.indexOf(u8, html, "Waiting for an available local processing worker") != null);
    try std.testing.expect(std.mem.indexOf(u8, html, "Starts automatically after the previous stage succeeds") != null);
    try std.testing.expect(std.mem.indexOf(u8, html, "Processing has not started yet") != null);
    try std.testing.expect(std.mem.indexOf(u8, html, "Your upload is saved") != null);
}
