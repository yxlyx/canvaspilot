...
connection.request("GET","/wiki/knowledge",headers={"Cookie":"cp_session=spy-token"}); response=connection.getresponse(); content=response.read(); assert response.status==200 and b"73%" in content
connection.close()
PY
