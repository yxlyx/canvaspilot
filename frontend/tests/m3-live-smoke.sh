...
class H(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  p=self.path
  if p.startswith("/api/outputs/page?"): return self.reply({"items":[output],"next_cursor":U})
  ...
  if p=="/api/enrollments": return self.reply([])
  ...
  if p.startswith("/api/wiki/activity"): return self.reply([])
  ...
  if p.startswith("/api/marked-papers/page?"): return self.reply({"items":[paper],"next_cursor":U})
  if p.startswith("/api/marked-papers?"): return self.reply([paper])
  if p.startswith("/api/marked-papers/"): return self.reply(paper)
  if p=="/api/providers": return self.reply([{"id":"codegraff","name":"Codegraff","models":["deepseek-v4-pro","claude-sonnet-4-6","mimo-v2.5"],"endpoint":"https://gateway.codegraff.com/v1","description":"One balance across several answer models.","capabilities":["Cited answers"],"setup_url":"https://codegraff.com/cli/auth","billing_note":"Credits are managed through Codegraff.","endpoint_mode":"fixed","supports_generation":True,"supports_embeddings":False,"recommended":True,"catalog_mode":"live","legacy":False},{"id":"openai","name":"OpenAI","models":["gpt-4o"],"endpoint":"https://api.openai.com/v1","description":"Grounded generation provider","capabilities":["Cited answers"],"setup_url":"https://platform.openai.com/api-keys","billing_note":"Developer API billing applies.","endpoint_mode":"fixed","supports_generation":True,"supports_embeddings":False,"recommended":False,"catalog_mode":"static","legacy":False}])
 ...
 def mutate(self):
  ...
  if self.headers.get("Authorization")=="Bearer expired-token": return self.reply({"detail":"expired"},401)
  if self.path=="/api/wiki/download" and self.headers.get("Authorization")=="Bearer bad-export-token": return self.reply({"detail":"not an archive"})
...
for i in $(seq 1 80); do
 ...
 if curl -fsS "http://127.0.0.1:$FRONTEND_PORT/login" 2>/dev/null | grep -q 'Continue your studies.'; then frontend_ready=true; break; fi
 sleep .1
done
...
check(){ body=$(curl -fsS -H "Cookie: $cookie" "$base$1"); case "$body" in *"$2"*) :;; *) printf '%s' "$body" >"$TMP/last-response.html"; echo "missing '$2' on $1" >&2; exit 1;; esac; }
check /wiki/guides "Grounded summary"; check /wiki/guides "?cursor=$U"; check /wiki/guides 'value="summary"'; check /wiki/guides 'value="outline"'; check /sources/papers "private.pdf"; check /sources/papers "?cursor=$U"; body=$(curl -fsS -H "Cookie: $cookie" "$base/flashcards?attempt=failed"); case "$body" in *'Your answer was not recorded; try again.'*) :;; *) echo "flashcard failure copy was not truthful" >&2; exit 1;; esac; case "$body" in *'local state'*) echo "flashcard failure claimed local state" >&2; exit 1;; *) :;; esac; check /dashboard "Live notes"; body=$(curl -fsS -H "Cookie: $cookie" "$base/dashboard"); case "$body" in *'No local module enrollments yet.'*) :;; *) echo "dashboard hid live data when modules were empty" >&2; exit 1;; esac; body=$(curl -fsS -H "Cookie: $cookie" "$base/settings/providers"); case "$body" in *'Connect Codegraff'*'Other providers'*) :;; *) echo "provider page did not render the guided connection flow" >&2; exit 1;; esac; case "$body" in *'name="endpoint"'*) echo "fixed provider endpoint is editable" >&2; exit 1;; *) :;; esac; check "/wiki/guides/$U" "Evidence snippet"; check /sources/health "Review more evidence"; check "/sources/health/$U" "Recommended action"; check /wiki/activity "No activity in this view"; check /wiki/knowledge "No module evidence yet"; check /sources/papers "private.pdf"; check "/sources/papers/$U" "Add a question manually"; check /settings/providers "Your sources remain available when no provider is connected."; check /settings/providers "Connect Codegraff"; check /settings/providers "Save and test"; check /settings/data 'value="wiki.export"'; check /wiki "Export Markdown workspace"; check /wiki 'value="wiki.export"'; body=$(curl -fsS -H "Cookie: $cookie" "$base/wiki"); case "$body" in *'0 decks'*'Live notes'*) :;; *) echo "live wiki inventory did not use live empty data" >&2; exit 1;; esac; case "$body" in *CS2030S*|*'Immutable Lists and Lazy Streams'*) echo "live wiki inventory rendered fixture data" >&2; exit 1;; *) :;; esac; check /wiki/live-notes "Download canonical Markdown"; body=$(curl -sS -H "Cookie: $cookie" "$base/wiki/not-generated"); case "$body" in *'No live wiki page has been generated for'*'Browse wiki pages'*) :;; *) echo "live missing wiki state was not truthful" >&2; exit 1;; *) :;; esac; case "$body" in *'Open demo wiki'*|*'No prototype wiki page'*) echo "live missing wiki state advertised demo content" >&2; exit 1;; *) :;; esac
# Explicit demo remains explicit; a dead live backend never falls back to its fixture title.
body=$(curl -fsS "$base/wiki/guides?mock=1"); case "$body" in *'data-cp-demo="true"'*"Grounded before generated."*) :;; *) exit 1;; esac
kill "$BPID"; wait "$BPID" 2>/dev/null || true; unset BPID
code=$(curl -sS -D "$TMP/live-error-headers" -o "$TMP/live-error-body" -w '%{http_code}' -H "Cookie: $cookie" "$base/wiki/guides"); test "$code" = 502; grep -qi '^cache-control: private, no-store' "$TMP/live-error-headers"; body=$(cat "$TMP/live-error-body"); case "$body" in *"Service unavailable"*) :;; *) exit 1;; esac; case "$body" in *"Immutable lists and streams"*) exit 1;; *) :;; esac
...
body=$(curl -fsS -H 'Cookie: cp_session=partial-token' "$base/wiki/guides"); case "$body" in *'value="source_ids" disabled'*"Sources are unavailable"*) :;; *) printf '%s' "$body" >"$TMP/last-response.html"; echo "partial dependency state missing" >&2; exit 1;; esac
...
connection.request("GET","/wiki/knowledge",headers={"Cookie":"cp_session=spy-token"}); response=connection.getresponse(); content=response.read(); assert response.status==200 and b"No module evidence yet" in content
connection.close()
PY
...
code=$(curl -sS -D "$TMP/page-expired-headers" -o /dev/null -w '%{http_code}' -H 'Cookie: cp_session=expired-token' "$base/wiki/guides"); test "$code" = 303; grep -qi 'location: /login' "$TMP/page-expired-headers"; grep -qi 'set-cookie: cp_session=;.*Max-Age=0' "$TMP/page-expired-headers"
...
curl -fsS -D "$TMP/form-all-headers" -o "$TMP/form-all.zip" -H "Cookie: $cookie" -H "Origin: $base" -H 'Sec-Fetch-Site: same-origin' -d 'action=wiki.export&selection=all' "$base/api/m3"; grep -qi 'content-disposition: attachment; filename="canonical-workspace.zip"' "$TMP/form-all-headers"; grep -qi 'content-type: application/zip' "$TMP/form-all-headers"; test -s "$TMP/form-all.zip"
curl -fsS -D "$TMP/form-selected-headers" -o "$TMP/form-selected.zip" -H "Cookie: $cookie" -H "Origin: $base" -H 'Sec-Fetch-Site: same-origin' -d 'action=wiki.export&selection=selected&page_ids=page%2Done&page%5Fids=page%2Btwo' "$base/api/m3"; grep -qi 'content-disposition: attachment; filename="canonical-workspace.zip"' "$TMP/form-selected-headers"; grep -Fq '{"page_ids":["page-one","page+two"]}' "$TMP/requests"; test -s "$TMP/form-selected.zip"
code=$(curl -sS -o /dev/null -w '%{http_code}' -H 'Cookie: cp_session=bad-export-token' -H "Origin: $base" -H 'Sec-Fetch-Site: same-origin' -H 'Content-Type: application/json' -d '{"action":"wiki.export","idempotency_key":"00000000-0000-4000-8000-000000000006","payload":{"page_ids":["11111111-1111-1111-1111-111111111111"]}}' "$base/api/m3"); test "$code" = 502
