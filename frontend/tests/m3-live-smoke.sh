...
class H(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if p.startswith("/api/marked-papers/"): return self.reply(paper)
  if p=="/api/providers": return self.reply([{"id":"openai","name":"OpenAI","models":["gpt-4o"],"endpoint":"https://api.openai.com/v1","description":"Grounded generation provider","capabilities":["Cited answers"],"setup_url":"https://platform.openai.com/api-keys","billing_note":"Developer API billing applies.","endpoint_mode":"fixed","supports_generation":True,"supports_embeddings":False}])
...
connection.request("GET","/wiki/knowledge",headers={"Cookie":"cp_session=spy-token"}); response=connection.getresponse(); content=response.read(); assert response.status==200 and b"recursion" in content and b"No legacy completion score." in content
connection.close()
PY
