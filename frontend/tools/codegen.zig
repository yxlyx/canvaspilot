...
pub fn main() !void {
    // Upstream toUrl() resizes its allocation in-place and the caller frees
    // the shorter slice, which trips DebugAllocator. Use an arena so we
    // don't care about per-allocation bookkeeping.
